...
[a1,a2,a3] = xlsread('171218_EFF_go2a',2);
a1(1,:) = [];
Th = a1(:,1)';
betaH(1,:) = a1(:,4); % H2O
betaH(2,:) = a1(:,5); % formmfr
betaH(3,:) = a1(:,6); % formh4mpt
betaH(4,:) = a1(:,7); % menylh4mpt
betaH(5,:) = a1(:,8); % mleneh4mpt
betaH(6,:) = a1(:,13); % mh4mpt
betaH(7,:) = a1(:,17); % mcom
betaH(8,:) = a1(:,19); % CH4
betaH(9,:) = (a1(:,20)+a1(:,21))./2; % F420
betaH(10,:) = a1(:,22); % HS-CoB
...
text(x1line+0.15,1.23,'60^oC','FontName','Myriad pro')
hold on
for i = 1:7
...
hold on
for i = 8:12
    plot(x1,alphaH(i,:),'Color',col(i-7,:))
    text((x1(1)+0.05),alphaHLoc(i),textLab2{i},'interpreter','latex')
    hold on
end
box off
xlabel('T [^oC]')
...
[c1,c2,c3] = xlsread('171218_EFF_comparison',2);
